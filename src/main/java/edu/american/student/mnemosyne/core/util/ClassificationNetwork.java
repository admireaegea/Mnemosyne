...
public class ClassificationNetwork
{
	...
	public static BasicNetwork addLayerToNetwork(BasicNetwork network,ClassificationNetworkConf conf, BasicLayer layer)
	{
		...
		double[] toInstallWeights = new double[toCopyWeights.length+(layer.getCount()*toCopyLayerCounts[toCopyLayerCounts.length-2])];
		...
		toInstallWeightIndex[toCopyLayerIndex.length]=toInstallWeightIndex[toCopyWeightIndex.length-1]+(layer.getCount()* toInstallLayerCounts[toCopyWeightIndex.length-1]);
		...
		for(int i=toCopyLayerOutput.length-(toCopy.getInputCount()+1);i<toInstallLayerOutput.length - (toCopy.getOutputCount()+2);i++)
		{
